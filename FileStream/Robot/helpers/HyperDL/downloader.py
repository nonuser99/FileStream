...
class HyperTGDownloader:
    def __init__(
        ...
        download_dir: str = "Hyperdownloads/",
        logger=None,
    ):
    ...
    async def start(self):
        try:
            ...
            print(f"❌ Failed to start helper bots: {e}")
    ...
    async def progress_callback(self, progress, progress_args):
        ...
        while not self._cancel_event.is_set():
            try:
                ...
                await asyncio.sleep(1)
            except (asyncio.CancelledError, StopTransmission):
                break
            except Exception:
                await asyncio.sleep(1)

    def merge_parts_sync(self, part_files, final_path):
        with open(final_path, "wb") as output:
            for _, part_path in sorted(part_files, key=lambda x: x[0]):
                ...
                try:
                    ...
                    self.logger(f"⚠️ Failed to delete part {part_path}: {e}")
    ...
    async def handle_download(self, progress, progress_args):
        ...
        if self.file_size < 10 * 1024 * 1024:
            num_parts = 1
        chunk_sizes = [self.file_size // num_parts] * num_parts
        for i in range(self.file_size % num_parts):
            chunk_sizes[i] += 1
        ...
        try:
            ...
            try:
                await loop.run_in_executor(None, partial(self.merge_parts_sync, results, temp_file_path))
            except Exception as e:
                self.logger(f"❌ Error merging part files: {e}")
                raise
            if prog_task and not prog_task.done():
                prog_task.cancel()
            final_path = ospath.abspath(self.final_path)
            await move(temp_file_path, final_path)
            return final_path
            ...
            self.logger(f"❌ HyperDL Error during download: {e}")
            return None
        finally:
    ...
    async def download_media(
        ...
        try:
            ...
            self.file_name = ospath.basename(file_name)
            self.directory = Path(work_dir or self.download_dir)
            self.final_path = file_name
